...
const validName = (name) => {
  ...
  if (name.length < stringLength) throw { status: UNPROCESSABLE_ENTITY_STATUS,
    err: {
      code: 'invalid_data',
...
const validQuantity = (quantity) => {
  if (quantity < 1) throw { status: UNPROCESSABLE_ENTITY_STATUS,
    err: {
      code: 'invalid_data',
...
const validNumber = (quantity) => {
  if (typeof quantity !== 'number') throw { status: UNPROCESSABLE_ENTITY_STATUS,
    err: {
      code: 'invalid_data',
