...
const deleteProductService = async (mongoId) => {
  try {
    const productData = await listProductByID(mongoId);
    
    await deleteProducts(mongoId);
    return productData;
  } catch (err) {
    throw validationError(err.status, err.message);
  }
};
